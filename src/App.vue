  ...
  methods: {
    ...
    fillPoly(poly) {
      ...
      for (let y = v0.y, i = 0; y < v1.y; y++, i++) {
        let intervalo = [arestas[0].ini.x + arestas[0].taxa * i, arestas[0].ini.x + arestas[2].taxa * i]
        lastX = intervalo[1]
        ...
        const delta = { r: (cor2.r - cor0.r) / varX, g: (cor2.g - cor0.g) / varX, b: (cor2.b - cor0.b) / varX }

        for (let j = intervalo[0], k = 0; j < intervalo[1]; j++, k++) {
          this.drawPixel(j, y, `rgb(${cor0.r + delta.r * k}, ${cor0.g + delta.g * k}, ${cor0.b + delta.b * k})`)
        }
      }

      swaped = false
      for (let y = v1.y, i = 0; y < v2.y; y++, i++) {
        let intervalo = [arestas[1].ini.x + arestas[1].taxa * i, lastX + arestas[2].taxa * i]
        if (intervalo[1] < intervalo[0]) {
          const aux = intervalo[0]
          intervalo[0] = intervalo[1]
          intervalo[1] = aux
        ...
        const varX = intervalo[1] - intervalo[0]
        const delta = { r: (cor2.r - cor1.r) / varX, g: (cor2.g - cor1.g) / varX, b: (cor2.b - cor1.b) / varX }

        for (let j = intervalo[0], k = 0; j < intervalo[1]; j++, k++) {
          this.drawPixel(j, y, `rgb(${cor1.r + delta.r * k}, ${cor1.g + delta.g * k}, ${cor1.b + delta.b * k})`)
        }
      }
